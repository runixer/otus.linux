...
resource "yandex_compute_instance" "bastion" {
  ...
  boot_disk {
    initialize_params {
      # Yandex NAT Instance (nat-instance-ubuntu-1612818947)
      # yc compute image list --folder-id standard-images | grep -i nat-instance
...
resource "yandex_lb_network_load_balancer" "lb" {
  ...
  # add kibana.otus.internal ip to /etc/hosts
  provisioner "local-exec" {
    command = "sudo sed -i '3i${tolist(tolist(self.listener)[0].external_address_spec)[0].address} kibana.otus.internal' /etc/hosts"
  }

  # remove kibana.otus.internal ip from /etc/hosts
  provisioner "local-exec" {
    when    = destroy
    command = "sudo sed -i '/${tolist(tolist(self.listener)[0].external_address_spec)[0].address} kibana.otus.internal/d' /etc/hosts"
  }

  # add kafkaui.otus.internal ip to /etc/hosts
  provisioner "local-exec" {
    command = "sudo sed -i '3i${tolist(tolist(self.listener)[0].external_address_spec)[0].address} kafkaui.otus.internal' /etc/hosts"
  }

  # remove kafkaui.otus.internal ip from /etc/hosts
  provisioner "local-exec" {
    when    = destroy
    command = "sudo sed -i '/${tolist(tolist(self.listener)[0].external_address_spec)[0].address} kafkaui.otus.internal/d' /etc/hosts"
  }

  # add consul.otus.internal ip to /etc/hosts
  provisioner "local-exec" {
    command = "sudo sed -i '3i${tolist(tolist(self.listener)[0].external_address_spec)[0].address} consul.otus.internal' /etc/hosts"
  }

  # remove consul.otus.internal ip from /etc/hosts
  provisioner "local-exec" {
    when    = destroy
    command = "sudo sed -i '/${tolist(tolist(self.listener)[0].external_address_spec)[0].address} consul.otus.internal/d' /etc/hosts"
  }
  ...
  content = templatefile("templates/ansible-inventory.ini",
    {
      ...
      log         = yandex_compute_instance.log.*
